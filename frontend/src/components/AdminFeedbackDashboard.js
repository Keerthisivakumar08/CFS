...
export default function AdminFeedbackDashboard() {
  ...
  // Removed filter state for all-time view
...
// Removed periodLabels and trendData - using dynamic monthData now

  // NEW: Dynamic all months labels and total data (no rate split)
  const monthData = useMemo(() => {
    ...
    return {
      labels: sortedMonths,
      totals: sortedMonths.map(month => monthCounts[month])
    };
  }, [filteredFeedback]);

  const stats = useMemo(() => {
    const positive = filteredFeedback.filter((f) => f.rating >= 4).length;
    const negative = filteredFeedback.filter((f) => f.rating <= 2).length;
    return { positive, negative, total: filteredFeedback.length };
  }, [filteredFeedback]);
        ...
        <Col xs={12} md={10} lg={10} className="p-4">
          <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between mb-4 gap-3">
            <div>
              ...
              <p className="section-subtitle mb-0">Overview of the latest client feedback and sentiment insights.</p>
            </div>
          ...
          <Row className="g-3 mb-4">
            <Col xs={12} md={4}>
              <div className="stats-card">
                <h5 className="chart-title mb-3">Analytics</h5>
                <Form.Select
                ...
                <div className="mt-4">
                  ...
                  <div className="d-flex justify-content-between mb-2">
                    <div className="text-muted">✅ Positive</div>
                    <div className="text-success fw-bold">{stats.positive}</div>
                  </div>
                  <div className="d-flex justify-content-between">
                    <div className="text-muted">❌ Negative</div>
                    <div className="text-danger fw-bold">{stats.negative}</div>
                  </div>
                </div>
              </div>
            </Col>
            ...
            <Col xs={12} md={4}>
              <div className="chart-container">
                <div className="chart-header">
                  <h5 className="chart-title mb-0">Positive Feedback</h5>
                  <span className="chart-badge bg-success-subtle text-success">{stats.positive}</span>
                </div>
                <AnalyticsChart
                  title="Positive"
                  labels={periodLabels}
            ...
            <Col xs={12} md={4}>
              <div className="chart-container">
                <div className="chart-header">
                  <h5 className="chart-title mb-0">Negative Feedback</h5>
                  <span className="chart-badge bg-danger-subtle text-danger">{stats.negative}</span>
                </div>
                <AnalyticsChart
                  title="Negative"
                  labels={periodLabels}
